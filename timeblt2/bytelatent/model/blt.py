...
def causal_mask(b, h, q_idx, kv_idx):
    ...
    Returns:
        bool: True if the query index is less than or equal to the key/value index
    """
    return q_idx >= kv_idx
...
class ByteLatentTransformer(nn.Module, SequenceModelWithOutput):
    ...
    def forward(
        ...
        patch_lengths = torch.full((bs, 12), 8).to('cuda')
        ...
        if not self.cross_attn_encoder:
            # print(f"cross_attn_encoder={self.cross_attn_encoder}")
            assert (
        ...
        # print(f"dec_embeds.shape={dec_embeds.shape}")

        # Generate decoder patch IDs
        # decoder_patch_ids = decoder_patch_ids_from_lengths(
        #     patch_lengths, nb_boe, local_decoder_tokens.shape[-1]
        # )
        # print(f"patch_ids.shape={patch_ids}")
        # print(f"decoder_patch_ids.shape={decoder_patch_ids}")
