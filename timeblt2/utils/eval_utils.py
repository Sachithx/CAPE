...
def evaluate_model_linear(model, input_len, pred_len, test_loader, test_tokenizer, patch_lengths, eval_batch_size, dataset_name, features, device='cuda'):
    """
    ...
    for i, (batch_x, batch_y, _, _) in pbar:
        try:
            ...
            with torch.no_grad():
                ...
                forecasts = model(token_ids.to(device), patch_lengths)
                actual = y.to(device)
            ...
            #             forecast_values = forecast_values[:min_shape[0]]
                ...
                mse = torch.mean((actual - forecasts) ** 2)
                mae = torch.mean(torch.abs(actual - forecasts))
                ...
                all_predictions.extend(forecasts.cpu().numpy().flatten())
                all_actuals.extend(actual.cpu().numpy().flatten())
...
def  evaluation(model, dataset_name, features, quant_range, vocab_size, input_len, pred_len, eval_batch_size, device='cuda'):
    ...
    results = evaluate_model(
        ...
        patch_lengths, eval_batch_size, dataset_name, features, device
    )
