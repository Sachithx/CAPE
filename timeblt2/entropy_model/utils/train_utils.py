...
def build_dataloader(dataset_name, features, seq_len, label_len, pred_len, flag, batch_size, pretrain):
    config = {
        ...
        'data': dataset_name,
        'embed': 'timeF',  # use time encoding
        'root_path': 'dataset/ETT-small/',  # adjust if your data is elsewhere
        'data_path': f'{dataset_name}.csv',
        'features': features,
        'seq_len': seq_len,
        'label_len': label_len,
        'pred_len': pred_len,
        'seasonal_patterns': None  # required by data_provider even if unused
    }

    args = argparse.Namespace(
        batch_size=batch_size,
        freq='h' if 'h' in dataset_name else 't',  # fix: 'm' → 't' is used by some ETT-minute configs
        num_workers=2,
        subsample_pct=None,
        fix_seed=42,
        target='OT',  # default for ETT datasets
        shuffle=True,
        scale=True
    )

    dataset, loader = data_provider(args, config, pretrain=pretrain, flag=flag)
...
def validate(model, data_loader, tokenizer, device, desc="Validation"):
