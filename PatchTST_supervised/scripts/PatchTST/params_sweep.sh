...
echo ""
echo "SHORT SEQUENCE EXPERIMENTS (seq_len=96)"
echo "========================================"

# Config 1: baseline_small - Conservative baseline
# cross_attn_k=2 means dim_global=2*dim_local (32=2*16)
run_experiment 96 128 512 16 2 2 32 16 2 0.0001 100 20 0.05 0.3 0.2 6 "baseline_small"
sleep 2

# Config 2: baseline_medium - Medium capacity
# cross_attn_k=2 means dim_global=2*dim_local (48=2*24)
run_experiment 96 96 384 16 2 3 48 24 2 0.00008 120 22 0.05 0.35 0.25 8 "baseline_medium"
sleep 2

# Config 3: baseline_large - Larger model
# cross_attn_k=2 means dim_global=2*dim_local (64=2*32)
run_experiment 96 80 320 16 3 3 64 32 2 0.00006 140 25 0.04 0.4 0.3 6 "baseline_large"
sleep 2

# Config 4: wide_architecture - Wider dimensions
# cross_attn_k=3 means dim_global=3*dim_local (96=3*32)
run_experiment 96 64 256 16 2 2 96 32 3 0.00005 130 25 0.06 0.32 0.22 8 "wide_architecture"
sleep 2

# Config 5: deep_local - More local layers
# cross_attn_k=2 means dim_global=2*dim_local (64=2*32)
run_experiment 96 80 320 12 4 2 64 32 2 0.00007 120 20 0.05 0.28 0.18 12 "deep_local"
sleep 2

# Config 6: deep_global - More global layers
# cross_attn_k=2 means dim_global=2*dim_local (48=2*24)
run_experiment 96 96 384 16 2 4 48 24 2 0.00008 130 25 0.04 0.35 0.25 6 "deep_global"
sleep 2

# Config 7: fine_patches - Small max patches, low thresholds
# cross_attn_k=2.5 means dim_global=2.5*dim_local (40=2.5*16)
run_experiment 96 112 448 8 2 3 40 16 2 0.00009 110 20 0.06 0.2 0.15 8 "fine_patches"
sleep 2

# Config 8: coarse_patches - Large max patches, high thresholds
# cross_attn_k=2 means dim_global=2*dim_local (64=2*32)
run_experiment 96 72 288 20 2 2 64 32 2 0.00006 140 30 0.05 0.6 0.4 4 "coarse_patches"
sleep 2

# Config 9: high_quant - Higher quantization range
# cross_attn_k=2 means dim_global=2*dim_local (48=2*24)
run_experiment 96 96 384 16 3 3 48 24 2 0.00007 125 22 0.05 0.35 0.25 12 "high_quant"
sleep 2

# Config 10: low_quant - Lower quantization range
# cross_attn_k=2 means dim_global=2*dim_local (32=2*16)
run_experiment 96 128 512 12 2 2 32 16 2 0.0001 110 20 0.06 0.3 0.2 4 "low_quant"
sleep 2

# Config 11: high_reg - Heavy regularization
# cross_attn_k=2 means dim_global=2*dim_local (48=2*24)
run_experiment 96 96 384 16 3 3 48 24 2 0.00008 120 18 0.1 0.25 0.15 6 "high_reg"
sleep 2

# Config 12: low_reg - Light regularization
# cross_attn_k=2 means dim_global=2*dim_local (64=2*32)
run_experiment 96 80 320 16 2 2 64 32 2 0.00012 100 25 0.02 0.45 0.35 8 "low_reg"
sleep 2
...
# Config 13: long_baseline - Conservative for long sequences
# cross_attn_k=2 means dim_global=2*dim_local (64=2*32)
run_experiment 336 64 256 16 3 3 64 32 2 0.00004 150 30 0.04 0.35 0.25 6 "long_baseline"
sleep 2

# Config 14: long_deep - Deep architecture for long context
# cross_attn_k=2.5 means dim_global=2.5*dim_local (80=2.5*32)
run_experiment 336 48 192 16 4 4 80 32 2 0.00003 180 35 0.04 0.4 0.3 8 "long_deep"
sleep 2

# Config 15: long_wide - Wide architecture for long context
# cross_attn_k=3 means dim_global=3*dim_local (96=3*32)
run_experiment 336 56 224 16 2 3 96 32 3 0.00005 160 28 0.05 0.38 0.28 6 "long_wide"
sleep 2

# Config 16: long_efficient - Efficient with coarse patches
# cross_attn_k=2 means dim_global=2*dim_local (48=2*24)
run_experiment 336 72 288 12 2 2 48 24 2 0.000035 170 32 0.045 0.5 0.35 4 "long_efficient"
sleep 2

# Config 17: long_fine - Fine-grained for long sequences
# cross_attn_k=2 means dim_global=2*dim_local (64=2*32)
run_experiment 336 60 240 8 3 3 64 32 2 0.000045 160 30 0.055 0.2 0.15 12 "long_fine"
sleep 2
...
echo "- cross_attn_k: Ratio of global_dim to local_dim (2, 2.5, or 3)"
...
echo "- Quantization range: 4, 6, 8, or 12"
echo "- Max patch length: 8, 12, 16, or 20"
